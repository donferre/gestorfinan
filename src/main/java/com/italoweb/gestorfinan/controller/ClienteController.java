...
public class ClienteController extends Window implements AfterCompose {
	...
	private void updateListitem(Cliente cliente, Listitem listitem) {
		...
		listitem.appendChild(new Listcell(
				cliente.getTipo() != null && cliente.getTipo().equals(TipoProveedor.NATURAL) ? "Natural" : "Juridico"));
		...
		if (this.usuarioSession.getRoles().getNombre().equalsIgnoreCase("ADMIN")) {
	...
	public void cargarWinClienteForm(Cliente cliente) {
		...
		if (cliente != null) {
			...
			for (Comboitem item : comb_tipo_proveedor.getItems()) {
				if (item.getValue() == cliente.getTipo()) {
